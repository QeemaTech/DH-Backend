...
class OrderResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'rating' => $this->when(
                $this->relationLoaded('orderRating') && $this->orderRating,
                fn () => [
                    'id' => $this->orderRating->id,
                    'rating' => $this->orderRating->rating,
                    'comment' => $this->orderRating->comment,
                    'created_at' => $this->orderRating->created_at?->toIso8601String(),
                ]
            ),
            'can_rate' => $this->status === 'delivered' && ! $this->orderRating,
        ];
    }
}
