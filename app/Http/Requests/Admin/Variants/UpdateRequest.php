...
class UpdateRequest extends FormRequest
{
            ...
            'options.*.code' => [
                ...
                function ($attribute, $value, $fail) {
                    if (! empty($value)) {
                        ...
                        $optionId = $this->input("options.{$index}.id");

                        $exists = \App\Models\VariantOption::where('code', $value)
                            ->when($optionId, fn ($q) => $q->where('id', '!=', $optionId))
                            ->exists();

                        if ($exists) {
                            $fail(__('This option code is already taken.'));
                        }
                    }
                },
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'options.*.id.exists' => __('One or more option IDs do not exist.'),
