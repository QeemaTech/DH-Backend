...
class CartController extends Controller
{
    ...
    {
        $this->ensureProductAvailableInUserCountry($product);
        // $request->validate([
    ...
    private function ensureProductAvailableInUserCountry(Product $product): void
    {
        $user = Auth::user();
        if (! $user->country_id) {
            return;
        ...
        if (! $product->isVisibleInCountry((int) $user->country_id)) {
            throw ValidationException::withMessages([
                'product' => [__('This product is not available in your country.')],
            ]);
        }
    }
