...
class ProductService
{
    ...
    protected DaftraService $daftraService;

    public function __construct(ProductRepository $productRepository, DaftraService $daftraService)
    {
    ...
    public function createProduct($request): Product
    {
        ...
        try {
            ...
            if ($request->has('country_ids') && is_array($request->country_ids)) {
                $this->productRepository->syncCountries($product, array_map('intval', $request->country_ids));
            }
    ...
    {
        $daftraProduct = $this->daftraService->getProductBySku($product->sku);

        if (! $daftraProduct) {
            throw new \Exception('Product not found in Daftra');
        }

        if (empty($daftraProduct['unit_price'])) {
            throw new \Exception('No price found in Daftra');
        }

        DB::transaction(function () use ($product, $daftraProduct) {

            $product->update([
                'price' => $daftraProduct['unit_price'],
                'daftr' => now(),
            ]);

            $product->branchProductStocks()->updateOrCreate(
                [
                    'branch_id' => 3,
                ],
                [
                    'quantity' => $daftraProduct['stock_balance'] ?? 0,
                ]
            );

            // if (!empty($daftraProduct['branches'])) {
            //     foreach ($daftraProduct['branches'] as $branch) {
