    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // Create/prepare hidden "Lists" sheet (prevents Excel repair issues caused by long inline list formulas)
                $listsSheetName = 'Lists';
                $listsSheet = $spreadsheet->getSheetByName($listsSheetName);
                if (! $listsSheet) {
                    $listsSheet = new PhpWorksheet($spreadsheet, $listsSheetName);
                    $spreadsheet->addSheet($listsSheet);
                }

                // 🔹 Vendors dropdown — Column A (vendor_id)
                $vendors = Vendor::select('id', 'name')->get();
                $vendorItems = $vendors->map(function ($v) {
                ...
                // Write vendor items to Lists!A
                $vendorRowStart = 1;
                foreach ($vendorItems as $idx => $value) {
                    $listsSheet->setCellValueExplicit('A'.($vendorRowStart + $idx), (string) $value, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
                }
                $vendorRowEnd = max($vendorRowStart, $vendorRowStart + $vendorItems->count() - 1);
                $spreadsheet->addNamedRange(new NamedRange('VendorsList', $listsSheet, '$A$'.$vendorRowStart.':$A$'.$vendorRowEnd));
                ...
                for ($row = 2; $row <= 1000; $row++) {
                    ...
                    $validation->setFormula1('=VendorsList');
                ...
                // Write category items to Lists!B
                $catRowStart = 1;
                foreach ($categoryItems as $idx => $value) {
                    $listsSheet->setCellValueExplicit('B'.($catRowStart + $idx), (string) $value, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
                }
                $catRowEnd = max($catRowStart, $catRowStart + $categoryItems->count() - 1);
                $spreadsheet->addNamedRange(new NamedRange('CategoriesList', $listsSheet, '$B$'.$catRowStart.':$B$'.$catRowEnd));
                ...
                // Hide the lists sheet (end users shouldn't touch it)
