...
use App\Http\Controllers\Api\SadadPaymentController;
...
Route::group(['middleware' => 'locale'], function () {
    ...
    Route::middleware('auth:sanctum')->group(function () {
        ...
        // Sadad payments (user) with rate limiting
        Route::post('payments/sadad/link', [SadadPaymentController::class, 'createPaymentLink'])
            // ->middleware('throttle:10,1')
            ->name('api.payments.sadad.link');
        Route::get('payments/invoices/{invoice}', [SadadPaymentController::class, 'showInvoice'])
            ->middleware('throttle:30,1')
            ->name('api.payments.invoices.show');
    ...
    Route::post('/digital-orders/{digitalOrder}/provider-order-test', [DigitalOrderController::class, 'providerOrderTest'])
        ->middleware('auth:sanctum')
        ->name('api.digital-orders.provider-order-test');
        
    Route::get('/digital-orders/{digitalOrder}/capture-ip', [DigitalOrderController::class, 'captureIp'])
        ->middleware('signed')
        ->name('api.digital-orders.capture-ip');
        
    Route::get('/digital-orders', [DigitalOrderController::class, 'index'])
        ->middleware('auth:sanctum')
        ->name('api.digital-orders.index');
        
    Route::get('/digital-orders/{digital_order}', [DigitalOrderController::class, 'show'])
        ->middleware('auth:sanctum')
        ->whereNumber('digital_order')
        ->name('api.digital-orders.show');
        
     Route::post('/digital-orders/{digital_order}/pay', [DigitalOrderController::class, 'pay'])
        ->middleware('auth:sanctum')
        ->whereNumber('digital_order')
        ->name('api.digital-orders.pay');
