...
class ApiFlowTest extends TestCase
{
    ...
    public function test_complete_user_order_flow(): void
    {
        ...
        $registeredUser->forceFill(['is_verified' => true])->save();

        // Step 3: Login (using 'login' field as per LoginRequest)
    ...
    public function test_rate_limiting_on_login(): void
    {
        ...
        for ($i = 1; $i <= 6; $i++) {
            ...
            if ($i <= 5) {
                ...
                $this->assertContains($response->status(), [422, 429], 'Login should be throttled after repeated failures.');
            }
        }
    }
    ...
    public function test_shipping_calculation(): void
    {
        ...
        $this->withToken($token)->postJson("/api/cart/{$product->id}")->assertStatus(201);
