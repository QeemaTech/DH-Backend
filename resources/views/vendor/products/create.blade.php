                        ...
                        <div x-data="productWizard()" x-init="init()">
                            ...
                            <div class="wizard-steps mb-4">
                                ...
                                <div x-show="currentStep === 2" x-transition.opacity class="wizard-content">
                                    ...
                                    <div x-show="formData.type === 'simple'" x-transition
                                        class="mb-4">
                                ...
                                <div x-show="currentStep === 3" x-transition.opacity class="wizard-content">
                                    ...
                                    <h6 class="mt-4 mb-2">{{ __('Country availability') }}</h6>
                                    <p class="text-muted small mb-2">{{ __('Optional. Leave all unchecked to show this product in every country.') }}</p>
                                    <div class="border rounded p-3" style="max-height: 220px; overflow-y: auto;">
                                        @foreach ($countries ?? [] as $c)
                                            <div class="form-check mb-2">
                                                <input type="checkbox" class="form-check-input" name="country_ids[]"
                                                    id="v_country_create_{{ $c->id }}" value="{{ $c->id }}"
                                                    {{ in_array($c->id, old('country_ids', [])) ? 'checked' : '' }}>
                                                <label class="form-check-label" for="v_country_create_{{ $c->id }}">
                                                    {{ $c->name[app()->getLocale()] ?? $c->name['en'] ?? $c->code }}
                                                    @if ($c->dial_code)
...
@push('scripts')
    <script>
        const registerVendorProductWizard = () => {
            Alpine.data('productWizard', () => ({
                ...
                init() {
                    // Set vendor_id from authenticated vendor
                    this.formData.vendor_id = '{{ $vendor->id ?? '' }}';
                    // Load branches automatically
                    this.loadVendorBranches();
                },
                ...
                async onVendorChange() {
                    // Load branches automatically for vendor
                    await this.loadVendorBranches();
                },

                async loadVendorBranches() {
                    try {
                        const response = await fetch(`/vendor/products/branches/by-vendor`, {
                            headers: {
        ...
        if (window.Alpine) {
            registerVendorProductWizard();
        } else {
            document.addEventListener('alpine:init', registerVendorProductWizard, {
                once: true
            });
        }
    </script>
