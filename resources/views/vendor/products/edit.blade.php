                                    ...
                                    @foreach ($countries ?? [] as $c)
                                        <div class="form-check mb-2">
                                            <input type="checkbox" class="form-check-input" name="country_ids[]"
                                                id="v_country_edit_{{ $c->id }}" value="{{ $c->id }}"
                                                {{ in_array($c->id, old('country_ids', $product->countries->pluck('id')->toArray())) ? 'checked' : '' }}>
                                            <label class="form-check-label" for="v_country_edit_{{ $c->id }}">
        ...
        window.productWizard = {
            ...
            init() {
                ...
                if (typeInput) {
                    ...
                    this.onTypeChange(); // Call once to set initial state
                }
            ...
            async loadVendorBranches() {
                try {
                    console.log('Loading vendor branches...');
                    ...
                    if (response.ok) {
                        ...
                        if (data.success && data.branches && Array.isArray(data.branches)) {
                            ...
                            // Update branch stocks for variations
                            this.variations.forEach(variation => {
                                if (!variation.branchStocks) variation.branchStocks = {};
                                this.vendorBranches.forEach(branch => {
            ...
            validateCurrentStep() {
                if (this.currentStep === 1) {
                    ...
                    return nameEn && nameEn.value && nameAr && nameAr.value;
