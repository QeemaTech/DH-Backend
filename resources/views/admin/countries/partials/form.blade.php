...
<div class="row g-3">
    ...
    <div class="col-md-4">
        <label class="form-label">{{ __('Verification Channel') }}</label>
        @php
            $selectedChannels = old('verification_channels', $country?->getVerificationChannels() ?? ['sms']);
        @endphp
        <select name="verification_channels[]" class="form-select @error('verification_channels') is-invalid @enderror" data-channel-multiselect multiple required>
...
<div class="mt-4 d-flex gap-2">
    ...
    <a href="{{ route('admin.countries.index') }}" class="btn btn-outline-secondary">{{ __('Cancel') }}</a>
</div>
