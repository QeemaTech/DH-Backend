    ...
    <div class="container-fluid p-4 p-lg-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
                ...
                <button type="button"
                        class="btn btn-outline-info sync-product-btn"
                        data-sync-url="{{ route('admin.products.sync', $product) }}">
                    <span class="sync-icon"><i class="bi bi-arrow-repeat me-2"></i></span>{{ __('Sync') }}
                </button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Sync product listener
    document.querySelectorAll('.sync-product-btn').forEach(button => {
        button.onclick = function() {
            const syncUrl = this.dataset.syncUrl;
            const btn = this;
            const originalHtml = btn.innerHTML;

            btn.disabled = true;
            btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>{{ __('Syncing...') }}';
            ...
            .then(async response => {
                ...
                if (!response.ok || data.success === false) {
                    throw new Error(data.message || '{{ __('Failed to sync product') }}');
                }
                return data;
            })
            .then(data => {
                Swal.fire({
                    icon: 'success',
                    title: '{{ __('Success') }}',
                    text: data.message || '{{ __('Product synced successfully.') }}',
                    timer: 1500,
                    showConfirmButton: false
                });
            ...
            .catch(error => {
                Swal.fire('{{ __('Error') }}', error.message || '{{ __('Failed to sync product') }}', 'error');
            })
            .finally(() => {
                btn.disabled = false;
