    ...
    <div class="container-fluid p-4 p-lg-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
                ...
                <button type="button" class="btn btn-outline-info" id="syncAllStockBtn" data-sync-url="{{ route('admin.products.sync-stock-all') }}">
                    <i class="bi bi-arrow-repeat me-2"></i>{{ __('Sync Stock') }}
                </button>
...
document.addEventListener('DOMContentLoaded', function () {
    const btn = document.getElementById('syncAllStockBtn');
    if (!btn) return;
    ...
    btn.addEventListener('click', function () {
        ...
        Swal.fire({
            title: '{{ __('Sync all products stock?') }}',
            text: '{{ __('This will queue a background job to sync all products stock and quantities from Daftra.') }}',
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: '{{ __('Yes, sync') }}',
            ...
            btn.disabled = true;
            btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>{{ __('Queuing...') }}';
            ...
            .then(async response => {
                ...
                if (!response.ok || data.success === false) {
                    throw new Error(data.message || '{{ __('Failed to queue sync job') }}');
                }
                return data;
            })
            .then(data => {
                Swal.fire({
                    icon: 'success',
                    title: '{{ __('Queued') }}',
                    text: data.message || '{{ __('Sync job has been queued successfully.') }}',
                    timer: 2000,
                    showConfirmButton: false
                });
            })
            .catch(error => {
                Swal.fire('{{ __('Error') }}', error.message || '{{ __('Failed to queue sync job') }}', 'error');
            })
            .finally(() => {
                btn.disabled = false;
