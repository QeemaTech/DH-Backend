        ...
        <div class="row">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <dl class="row mb-0">
                            ...
                            <dd class="col-sm-9 mt-3">{{ number_format($order->sub_total, 2) }} {{ setting('currency', 'KWD') }}</dd>

                            <dt class="col-sm-3 mt-3">{{ __('Order Discount') }}</dt>
                            <dd class="col-sm-9 mt-3">{{ number_format($order->order_discount, 2) }} {{ setting('currency', 'KWD') }}</dd>

                            @if($order->coupon)
                                <dt class="col-sm-3 mt-3">{{ __('Coupon') }}</dt>
                                <dd class="col-sm-9 mt-3">
                                    <span class="badge bg-info">{{ $order->coupon->code }}</span>
                                    <span class="text-muted ms-2">(-{{ number_format($order->coupon_discount, 2) }} {{ setting('currency', 'KWD') }})</span>
                                </dd>
                            @endif

                            <dt class="col-sm-3 mt-3">{{ __('Shipping') }}</dt>
                            <dd class="col-sm-9 mt-3">{{ number_format($order->total_shipping, 2) }} {{ setting('currency', 'KWD') }}</dd>
                            ...
                            @if($order->wallet_used > 0)
                                ...
                                <dd class="col-sm-9 mt-3">
                                    <span class="badge bg-primary">{{ number_format($order->wallet_used, 2) }} {{ setting('currency', 'KWD') }}</span>
                                </dd>
                            @endif

                            <dt class="col-sm-3 mt-3">{{ __('Total') }}</dt>
                            <dd class="col-sm-9 mt-3">
                                <strong class="fs-5">{{ number_format($order->total, 2) }} {{ setting('currency', 'KWD') }}</strong>
                            </dd>

                            @if($order->total_commission > 0)
                                <dt class="col-sm-3 mt-3">{{ __('Commission') }}</dt>
                                <dd class="col-sm-9 mt-3">
                                    <span class="badge bg-success">{{ number_format($order->total_commission, 2) }} {{ setting('currency', 'KWD') }}</span>
                                </dd>
                            @endif
                                    ...
                                    <div class="card-body">
                                        <div class="row mb-3">
                                            <div class="col-md-6">
                                                ...
                                                <strong>{{ number_format($vendorOrder->sub_total, 2) }} {{ setting('currency', 'KWD') }}</strong>
                                            </div>
                                            <div class="col-md-6">
                                                <small class="text-muted">{{ __('Shipping') }}:</small>
                                                <strong>{{ number_format($vendorOrder->shipping_cost, 2) }} {{ setting('currency', 'KWD') }}</strong>
                                            </div>
                                            @if($vendorOrder->discount > 0)
                                                <div class="col-md-6 mt-2">
                                                    <small class="text-muted">{{ __('Discount') }}:</small>
                                                    <strong class="text-danger">-{{ number_format($vendorOrder->discount, 2) }} {{ setting('currency', 'KWD') }}</strong>
                                                </div>
                                            @endif
                                            @if($vendorOrder->commission > 0)
                                                <div class="col-md-6 mt-2">
                                                    <small class="text-muted">{{ __('Commission') }}:</small>
                                                    <strong class="text-success">{{ number_format($vendorOrder->commission, 2) }} {{ setting('currency', 'KWD') }}</strong>
                                                </div>
                                            @endif
                                            <div class="col-md-12 mt-2">
                                                <small class="text-muted">{{ __('Total') }}:</small>
                                                <strong class="fs-5">{{ number_format($vendorOrder->total, 2) }} {{ setting('currency', 'KWD') }}</strong>
                                            </div>
                                        </div>
                                                ...
                                                <tbody>
                                                    @foreach($vendorOrder->items as $item)
                                                        <tr>
                                                            ...
                                                            <td class="text-end">{{ number_format($item->price, 2) }} {{ setting('currency', 'KWD') }}</td>
                                                            <td class="text-end"><strong>{{ number_format($item->total, 2) }} {{ setting('currency', 'KWD') }}</strong></td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                            </table>
            ...
            <div class="col-lg-4">
                ...
                @if($order->orderRating)
                    <div class="card mb-3">
                        <div class="card-header">
                            <h5 class="card-title mb-0">{{ __('Order Rating') }}</h5>
                        </div>
                        <div class="card-body">
                            <div class="d-flex align-items-center mb-2">
                                @for($i = 1; $i <= 5; $i++)
                                    <i class="bi bi-star{{ $i <= $order->orderRating->rating ? '-fill' : '' }} text-warning fs-5 me-1"></i>
                                @endfor
                                <span class="ms-2 fw-bold">{{ $order->orderRating->rating }}/5</span>
                            </div>
                            @if($order->orderRating->comment)
                                <p class="mb-0 text-muted small">{{ $order->orderRating->comment }}</p>
                            @endif
                            <small class="text-muted">{{ $order->orderRating->created_at->format('M d, Y H:i') }}</small>
                        </div>
                    </div>
                @endif
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <dl class="mb-0">
                            ...
                            <dd><strong>{{ number_format($order->total, 2) }} {{ setting('currency', 'KWD') }}</strong></dd>
